...
const Sidebar = ({ isSidebarOpen, chatHistory }: { isSidebarOpen: boolean, chatHistory: chatHistoryType[] }) => {
    return (
        <aside className={`${isSidebarOpen ? 'w-72' : 'w-0 overflow-hidden'} transition-all duration-300 bg-[#1e1f20] flex flex-col h-screen`}>
            <div className="p-4 flex-none">
                <button className="flex items-center gap-3 px-4 py-3 bg-[#1a1c1e] hover:bg-[#282a2c] rounded-full text-sm transition-colors mb-4 w-fit">
